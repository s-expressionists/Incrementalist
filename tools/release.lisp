...
(cl:defpackage #:incrementalist.tools.release
  (:use
   #:cl
   #:alexandria
   #:split-sequence)
  ...
  (:local-nicknames
   ...
   (#:changes #:incrementalist.tools.read-changes)
   (#:news    #:incrementalist.tools.write-news)))

(cl:in-package #:incrementalist.tools.release)
...
(defun message (format-control &rest format-arguments)
  (format *trace-output* "~V@T~?~%"
          (* 2 *level*) format-control format-arguments))
